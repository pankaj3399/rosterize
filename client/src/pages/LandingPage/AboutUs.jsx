        ...
        <div className="grid-cols-1 w-full">
          ...
          <div className="text-white mt-[30px]">
            ...
            <div className="flex flex-col">
              {data.plans.map((plan) => (
                <div key={plan._id} className="bg-[#0E2442] text-white p-4 mt-4">
                  <h3 className="text-[18px] font-bold">{plan.name}</h3>
                  <p className="sm:text-[16px] mt-1">Number of Employees: {plan.range}</p>
          ...
          <div className="text-white mt-[30px]">
...

            {
              data.reviews.map((review) => (
                <div key={review._id} className="bg-[#0E2442] text-white p-4 mt-4">
                  <p className="sm:text-[16px] mt-1">
                    <span className="underline">“{review.review}”</span>{" "}
                    -  {review.user.role}, {review.user.firstName || review.company.name}{" "}
                  </p>
                </div>
              ))
            }
